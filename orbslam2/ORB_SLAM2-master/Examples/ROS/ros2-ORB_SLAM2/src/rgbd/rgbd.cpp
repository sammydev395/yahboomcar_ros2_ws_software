...
int main(int argc, char **argv)
{
    ...
    // Create SLAM system using shared_ptr to avoid double free
    bool visualization = true;
    auto slamSystem = std::make_shared<ORB_SLAM2::System>(argv[1], argv[2], ORB_SLAM2::System::RGBD, visualization);

    auto node = std::make_shared<RgbdSlamNode>(slamSystem);
    rclcpp::spin(node);
